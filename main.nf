...
workflow {
    ...
    if (!params.skip_dereplication) {
        if (!params.skip_filter) {
            ...
            drep_wo_genomeinfo(genomes_noid_ch.collect())
            derep_info(drep_wo_genomeinfo.out.cdb, drep_wo_genomeinfo.out.wdb)
        }
    }

    if (!params.skip_taxonomy) {
        if (params.gtdbtk_db == 'none') {
            gtdbtk_db()
            gtdbtk_db = gtdbtk_db_download.out.gtdbtk_db
        }
        else {
            gunc_db = file(params.gtdbtk_db, type: 'dir', checkIfExists: true)
        }

        if (!params.skip_filter) {
            gtdbtk_classify_wf(genome_filter.out.genomes.collect(), gtdbtk_db)
        } else {
            gtdbtk_classify_wf(genomes_noid_ch.collect())
        }
    }
}
